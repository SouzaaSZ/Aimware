local function CheckForUpdates()
    ...
    local script_url =
        "https://raw.githubusercontent.com/SouzaaSZ/Aimware/refs/heads/main/Lua/%5BHELPER%5D%20SwitzerHook%20.lua"
...
local function GetProcessNameFromWindow(hwnd)
    if not hwnd or hwnd == ffi.cast("void*", 0) then
        return ""
    end

    local pid_ptr = ffi.new("unsigned long[1]")
    ffi.C.GetWindowThreadProcessId(hwnd, pid_ptr)
    local pid = pid_ptr[0]
    ...
    local success = ffi.C.Process32First(snapshot, pe)
    while success ~= 0 do
        if pe.th32ProcessID == pid then
            local exe_name = ffi.string(pe.szExeFile)
            ffi.C.CloseHandle(snapshot)
            return exe_name:lower()
        end
        success = ffi.C.Process32Next(snapshot, pe)
    end
...
local function IsGameInForeground()
    ...
    if not active_window or active_window == ffi.cast("void*", 0) then
        return false
    end

    local process_name = GetProcessNameFromWindow(active_window)
    return process_name == "cs2.exe"
end
...
switzer_ui.info_text = gui.Text(switzer_ui.infor_lua,
...
local function RunAimbotLogic(lp)
    ...
    UpdateStateIfChanged("autofire", switzer_vars.aftoggler:GetValue(), function(v)
        gui_set("lbot.trg.autofire", v)
    end)
    UpdateStateIfChanged("autowall", switzer_vars.aptoggler:GetValue(), function(v)
        SetAllWeaponsOption("autowall", v and 1 or 0)
    end)
    UpdateStateIfChanged("smoke", switzer_vars.smktoggle:GetValue(), function(v)
        SetAllWeaponsOption("smoke", v and 1 or 0)
    end)
    UpdateStateIfChanged("nospread", switzer_vars.nstoggler:GetValue(), function(v)
        gui_set("lbot.trg.weapon.shared.accuracy.antispread", v)
    end)
...
local function UpdateViewModel(lp)
    ...
    UpdateStateIfChanged("view_x", is_custom and switzer_vars.SLIDER_VIEWX:GetValue() or xview, function(v)
        client.SetConVar("viewmodel_offset_x", v, true)
    end)
    UpdateStateIfChanged("view_y", is_custom and switzer_vars.SLIDER_VIEWY:GetValue() or yview, function(v)
        client.SetConVar("viewmodel_offset_y", v, true)
    end)
    UpdateStateIfChanged("view_z", is_custom and switzer_vars.SLIDER_VIEWZ:GetValue() or zview, function(v)
        client.SetConVar("viewmodel_offset_z", v, true)
    end)
end
...
local function DrawWatermark()
    ...
    if CACHE["wt_uid"] ~= cur_uid or CACHE["wt_build"] ~= cur_build then
        ...
        if cur_uid then
            cached_watermark_text = cached_watermark_text .. " | Uid: " .. user.id
        end
        if cur_build then
            cached_watermark_text = cached_watermark_text .. " | " .. user.build
        end
...
xpcall(function()
    ...
    callbacks.Register("Draw", function()
        ...
        UpdateStateIfChanged("trg_delay", delay, function(v)
            for i = 1, #weapons_default do
                gui_set("lbot.trg.weapon." .. weapons_default[i] .. ".delay", v)
            end
        end)
