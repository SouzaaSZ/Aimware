...
local function GetProcessNameFromWindow(hwnd)
    if not hwnd or hwnd == ffi.cast("void*", 0) then return "" end
    ...
    if snapshot == ffi.cast("void*", -1) then return "" end
...
local function IsGameInForeground()
    ...
    if not active_window or active_window == ffi.cast("void*", 0) then return false end
